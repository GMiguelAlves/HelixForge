...
| 9B2-D05 | `ENVIRONMENTAL_RECOVERY` | The Stage 9B.1 audit preserved the certified Temurin 21.0.12+8 identity, but its portable binary was not present in the retained runtime paths when Stage 9B.2 began. | Restore only the same official Eclipse Temurin archive in the dedicated Stage 9B.2 scratch root, verify its official SHA-256, and repeat the exact runtime preflight before launching Nextflow. No scientific package or parameter changes. |
