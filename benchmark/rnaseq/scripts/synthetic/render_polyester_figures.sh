...
"$python_bin" "$scripts/prepare_polyester_figures.py" \
    --truth-dir "$truth" \
...
"$rscript_bin" "$scripts/plot_polyester_benchmark.R" "$output_dir/data" "$output_dir"
{
    "$python_bin" --version
    "$rscript_bin" --version
} > "$output_dir/render_versions.txt" 2>&1
"$python_bin" "$scripts/finalize_polyester_figures.py" "$output_dir"
