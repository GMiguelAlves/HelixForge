...
process PREPARE_SYNTHETIC_NARROW {
    tag 'synthetic-narrow:reference-truth'
...
process BUILD_MAPPABILITY_INDEX {
    tag 'synthetic-narrow:mappability-index'
...
process VALIDATE_TRUTH_MAPPABILITY {
    tag 'synthetic-narrow:truth-mappability'
    label 'benchmark_low'
    cache 'deep'
    errorStrategy 'terminate'
    maxRetries 0
...
process SIMULATE_CHIPS_LIBRARY {
    ...
    python '${simulation_script}' \
        ...
        --peaks '${truth}/narrow_true_peaks.bed' \
        --sample '${sample}' --output-dir library
    """
}


process VALIDATE_SYNTHETIC_NARROW_DATASET {
    tag 'synthetic-narrow:dataset-validation'
    label 'benchmark_medium'
    cache 'deep'
    errorStrategy 'terminate'
    maxRetries 0
