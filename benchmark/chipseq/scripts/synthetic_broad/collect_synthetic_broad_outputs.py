#!/usr/bin/env python3
"""Select and validate technical outputs used by the synthetic broad evaluator."""
...
def parse_trace(path: Path) -> dict:
    ...
    return {
        "tasks": len(rows),
        "cached_tasks": sum(row.get("cached") == "true" for row in rows),
        "processes": sorted({row.get("process", "") for row in rows}),
    }
...
def consensus_candidates(root: Path) -> list[Path]:
    return [
        path
        for path in root.rglob("consolidated_peaks.bed")
        if path.parent.name.endswith(".replicate_support.consensus_result")
    ]
...
def main() -> None:
    ...
    for sample in ("chip_rep1", "chip_rep2"):
        selected[f"helixforge_{sample}"] = require_one(
            list((results / "080-peak-calling").glob(f"*{sample}*.peak_calling/peaks.broadPeak")),
            f"HelixForge {sample} broadPeak set",
        )
        selected[f"independent_{sample}"] = require_one(
            list((independent / "peaks" / sample).glob("*_peaks.broadPeak")),
            f"independent {sample} broadPeak set",
        )
    selected["helixforge_consensus"] = require_one(
        consensus_candidates(results / "chipseq" / "consensus"),
        "HelixForge replicate-support consensus",
    )
    selected["independent_consensus"] = require_one(
        [independent / "consensus" / "consolidated_peaks.bed"],
        "independent replicate-support consensus",
    )
    final_bams = {}
    for sample in ("chip_rep1", "chip_rep2"):
        final_bams[f"helixforge_{sample}"] = require_one(
            list((results / "060-filtering").rglob(f"{sample}*.filtered.bam")),
            f"HelixForge {sample} final BAM",
        )
        final_bams[f"independent_{sample}"] = require_one(
            [independent / "bam" / f"{sample}.final.bam"],
            f"independent {sample} final BAM",
        )
    ...
    consolidation = require_one(
        [results / "chipseq/consensus/consolidation_summary.json"],
        "consensus terminal summary",
    )
    consolidation_document = json.loads(consolidation.read_text(encoding="utf-8"))
    if consolidation_document.get("status") not in {"complete", "complete_empty"}:
        raise ValueError("consensus terminal summary is incomplete")
    ...
    peak_rows = [
        {"role": role, "path": str(path), "sha256": sha256(path), "records": count_records(path), "size_bytes": path.stat().st_size}
        for role, path in selected.items()
    ]
    with (output / "selected_region_sets.tsv").open("w", encoding="utf-8", newline="") as handle:
        writer = csv.DictWriter(handle, fieldnames=list(peak_rows[0]), delimiter="\t", lineterminator="\n")
        writer.writeheader()
        writer.writerows(peak_rows)
    bam_rows = [{"role": role, "path": str(path), "size_bytes": path.stat().st_size} for role, path in final_bams.items()]
    with (output / "selected_bams.tsv").open("w", encoding="utf-8", newline="") as handle:
        writer = csv.DictWriter(handle, fieldnames=list(bam_rows[0]), delimiter="\t", lineterminator="\n")
        writer.writeheader()
    ...
    technical_rows = [
        ...
        {"check": "replicate_broad_regions", "status": "PASS", "value": "2 non-empty broadPeak sets"},
        {"check": "replicate_support_consensus", "status": "PASS", "value": f"{count_records(selected['helixforge_consensus'])} final regions"},
        ...
        {"check": "terminal_consensus_manifest", "status": "PASS", "value": str(consolidation)},
    ...
    manifest = {
        "schema_version": "1.0",
        "type": "synthetic_broad_selected_outputs",
        "status": "TECHNICAL_EXECUTION_PASS",
        "session_uuid": session.group(1) if session else None,
        "trace": trace_metrics,
        "artifacts": {role: {"path": str(path), "sha256": sha256(path), "records": count_records(path)} for role, path in selected.items()},
        "bams": {role: {"path": str(path), "size_bytes": path.stat().st_size} for role, path in final_bams.items()},
        "frip": frip_rows,
        "multiqc": {"path": str(multiqc), "sha256": sha256(multiqc)},
        "nextflow_report": {"path": str(nextflow_report), "sha256": sha256(nextflow_report)},
        "terminal_consensus": {"path": str(consolidation), "sha256": sha256(consolidation)},
    }
