...
dataset_root=${3:?dataset root is required}
runtime_prefix=${4:?frozen ChIP runtime prefix is required}
java_home=${5:?Java 21 home is required}
nextflow_launcher=${6:?Nextflow launcher is required}
queue=${7:-general}
mode=${8:-run}
...
[[ -z "${SLURM_JOB_ID:-}" ]] || {
    echo "The Nextflow driver must run on the head node, not inside a Slurm allocation." >&2
    exit 2
}
...
[[ "$(git -C "$repo_root" merge-base HEAD "$amendment_commit")" == "$amendment_commit" ]]
[[ -x "$runtime_prefix/bin/bowtie2" ]]
[[ -x "$runtime_prefix/bin/samtools" ]]
[[ -x "$runtime_prefix/bin/macs3" ]]
[[ -x "$java_home/bin/java" ]]
[[ -x "$nextflow_launcher" ]]
[[ -s "$dataset_root/provenance/dataset_manifest.json" ]]
...
runtime_version=$($nextflow_launcher -version 2>&1)
[[ "$runtime_version" == *"version 25.10.7"* ]] || {
    printf 'Expected Nextflow 25.10.7, observed:\n%s\n' "$runtime_version" >&2
    exit 3
}
...
if [[ "$mode" == "resume" ]]; then
    ...
    python "$repo_root/benchmark/chipseq/scripts/synthetic_broad/prepare_helixforge_broad_inputs.py" \
        --dataset-root "$dataset_root" --run-root "$run_root/input"
else
...
command=(
    ...
    --chipseq_peak_type broad
    --chipseq_effective_genome_size 54000000
    ...
    --chipseq_frip_blacklist_policy none
...
printf '%s\n' "$amendment_commit" > "$run_root/protocol_amendment_commit.txt"
