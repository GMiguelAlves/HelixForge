...
idr_prefix=${5:?IDR runtime prefix is required}
...
[[ -x "$idr_prefix/bin/idr" ]]
[[ -x "$java_home/bin/java" ]]
[[ -x "$nextflow_launcher" ]]
[[ -s "$dataset_root/provenance/dataset_manifest.json" ]]

export JAVA_HOME="$java_home"
export PATH="$runtime_prefix/bin:$idr_prefix/bin:$java_home/bin:/usr/bin:/bin"
...
[[ "$(idr --version 2>&1 | head -n 1)" == "IDR 2.0.4.2" ]]
...
if [[ "$mode" == "resume" ]]; then
    ...
    python "$repo_root/benchmark/chipseq/scripts/synthetic_narrow/prepare_helixforge_narrow_inputs.py" \
