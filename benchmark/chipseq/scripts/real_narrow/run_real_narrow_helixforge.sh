...
expected_root=/scratch/HELIXFORGE_WORKSPACE/helixforge-chipseq-real-narrow-benchmark-20260830
...
[[ -x "$idr_prefix/bin/idr" && -x "$java_home/bin/java" && -x "$nextflow_launcher" ]]
[[ -s "$benchmark_root/preflight/preflight.json" ]]
...
if [[ "$mode" == resume ]]; then
    ...
    "$runtime_prefix/bin/python" "$repo_root/benchmark/chipseq/scripts/real_narrow/prepare_helixforge_real_narrow_inputs.py" \
...
command=(
    ...
    --chipseq_peak_caller macs3 --chipseq_peak_type narrow
    ...
    --chipseq_idr_threshold 0.05 --chipseq_idr_rank_metric signal_value
    ...
    --macs3_queue "$queue" --peak_qc_queue "$queue"
    --consensus_queue "$queue" --idr_queue "$queue"
