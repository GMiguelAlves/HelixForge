...
`--chipseq_run_mode peaks` uses the native API when `--chipseq_native_peak_calling true`. When false, only the legacy `peaks` step is invoked. `full` remains the complete legacy fallback; native and legacy callers are never run simultaneously for one mode.
