...
| SD-004 | Cache | Identical deep fingerprints were submitted again under `-resume`. | Reproducibility is intact, but compute reuse and invalidation claims are unproven. | OPEN-CONDITIONAL | Repeat on native Linux and production Slurm storage with the supported Nextflow version. |
