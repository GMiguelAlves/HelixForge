process DB_PREFLIGHT {
    ...
    printf '%s\n' '{"analysis_id":"stub.condition","model_id":"stub.condition.deseq2","id":"control_vs_treated","factor":"condition","numerator":"control","denominator":"treated","alpha":0.05,"lfc_threshold":0,"order":2}' > contrast_specs/stub.condition--control_vs_treated.json
    printf '%s\n' '{"schema_version":"1.0","type":"differential_binding_request","analysis_id":"stub.condition","peak_bed":"stub.condition.bed","sample_table":"stub.condition.tsv","count_spec":"stub.condition.count.json","model_spec":"stub.condition.model.json","contrast_specs":["stub.condition--treated_vs_control.json","stub.condition--control_vs_treated.json"],"status":"stub"}' > analysis_requests/stub.condition.json
    printf 'analysis_id\ttarget\tgenome_id\tpeak_type\tconditions\tsamples\tpeaks\tcontrasts\tstatus\nstub.condition\tH3K27ac\tstub\tnarrow\tcontrol,treated\t2\t1\t2\tstub\n' > db_preflight_summary.tsv
    printf '{"status":"stub","analyses":1,"contrasts":2}\n' > db_preflight_report.json
