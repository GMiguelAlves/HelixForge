...
def build_evidence(manifest: dict[str, Any], bindings: dict[str, Path], output: Path) -> dict[str, Any]:
    ...
    document = {"schema_version": "1.0", "evidence_model_version": "1.0", "type": "evidence_manifest", "id": f"{manifest['id']}.evidence", "assay": assay, "run_id": manifest["run"]["run_id"], "reference_id": manifest["reference"]["reference_id"], "source_run_manifest_id": manifest["id"], "status": "complete" if datasets else "complete_empty", "contrasts": manifest.get("contrasts", []), "datasets": datasets, "artifact_catalog": catalog, "provenance": {"provider": f"{assay}_evidence_provider", "provider_version": "1.0", "source_run_manifest_id": manifest["id"]}}
