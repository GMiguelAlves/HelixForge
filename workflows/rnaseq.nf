...
include { RNASEQ_REPORT as RNASEQ_REPORT_REENTRY } from '../subworkflows/local/rnaseq/report'
...
workflow RNASEQ {
    ...
    if (!(run_mode in ['qc', 'alignment', 'quant', 'quantification', 'import', 'de', 'differential_expression', 'report', 'report_reentry', 'full'])) {
        error "Invalid rnaseq_run_mode '${params.rnaseq_run_mode}'. Use qc, alignment, quantification, import, de, report, report_reentry, or full."
    }

    terminal_manifest = channel.empty()
    terminal_bundle = channel.empty()
    if (run_mode == 'report_reentry') {
        required_inputs = [
        ...
        missing_inputs = required_inputs.findAll { name -> !params[name] }
        if (missing_inputs) {
            error "rnaseq_run_mode=report_reentry requires: ${missing_inputs.join(', ')}"
        }
        ...
        import_manifest = file(params.rnaseq_report_import_manifest, checkIfExists: true)
        abundance = file(params.rnaseq_report_abundance, checkIfExists: true)
        samples = file(params.rnaseq_report_samples, checkIfExists: true)
        annotation = file(params.rnaseq_report_annotation, checkIfExists: true)
        de_results = file(params.rnaseq_report_de_results, checkIfExists: true)
        de_manifest = file(params.rnaseq_report_de_manifest, checkIfExists: true)
        genes = file(params.rnaseq_report_genes, checkIfExists: true)
        import_document = new groovy.json.JsonSlurper().parse(import_manifest.toFile())
        de_document = new groovy.json.JsonSlurper().parse(de_manifest.toFile())
        ...
        report_meta = [
            id         : 'rnaseq.report.reentry.candidate_genes',
            provider   : 'candidate_genes_v1',
            target_dir : report_target,
            import_id  : import_document.id?.toString(),
            analysis_id: de_document.id?.toString(),
            reentry    : true
        ]
        report_request = channel.value(tuple(
            report_meta, import_manifest, abundance, samples, annotation,
            de_results, de_manifest, genes, report_parameters_base64
        ))
        RNASEQ_REPORT_REENTRY(report_request)
        completed_status = RNASEQ_REPORT_REENTRY.out.status
        workflow_logs = RNASEQ_REPORT_REENTRY.out.reports
    } else {
        ...
        workflow_logs = RNASEQ_NATIVE_FOUNDATION.out.logs
