...
def run_case(name: str, mutate, expected_success: bool, expected_text: str = "") -> None:
    with tempfile.TemporaryDirectory(prefix=f"omicsflow-de-{name}-") as temporary:
        ...
        result = subprocess.run([
            "python3", str(PROGRAM), "--manifest", str(manifest_path),
...
def missing_design(_counts, _samples, spec):
    document = json.loads(spec.read_text())
    document["test_variables"] = ["missing_field"]
    spec.write_text(json.dumps(document))
...
run_case("unsupported-lrt", lrt, False, "only the legacy Wald test")
run_case("missing-design", missing_design, False, "none of the configured test variables")
